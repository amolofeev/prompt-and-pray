...
> **Исторический трекер подпроекта `yt`.** Подпроект изолирован в `yt/` и готовится к
> переносу в отдельный git-репозиторий (#53); после переноса трекер продолжается там.
> Здесь — архив истории этапов #5–#52 монорепо.
...
**Последнее обновление:** 2026-08-02 (изоляция yt в `yt/`, подготовка переноса #53).
