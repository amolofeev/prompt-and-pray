...
> **Исторический документ.** Подпроект `yt` изолирован в `yt/` и готовится к переносу
> в отдельный git-репозиторий (#53). Актуальная копия — `yt/docs/PLAN.md` (R10); эта —
> архив для истории монорепо.
