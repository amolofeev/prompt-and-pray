...
(аргумент `path` или паттерн с префиксом `yt/`):

- Вопрос про процесс/доки/issues/корень → ищи в корне, в поддиректорию `yt/` не заходи.
- Вопрос про Go-код/команды yt → ищи внутри `yt/` с `path: yt`; свой код там —
  `yt/internal`, `yt/cmd`.
...
`yt/vendor/`, `yt/bin/`, `.opencode/node_modules/`. Они не в git и воссоздаются по мере
надобности (`go mod vendor`, `make build`, `npm install`).
...
- Начиная работу над подпроектом, СНАЧАЛА прочитай его `AGENTS.md`: там локальные правила
  и ссылки на локальную документацию (например, устройство `yt/` — `yt/docs/structure.md`).
...
6. Коммит в формате `[AI] #N <описание>` (ссылка на задачу `#N` сразу после блока
   `[AI]`, до текста), изменения запушены в origin.
