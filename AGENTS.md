...
2. Тесты добавлены и проходят: юнит (валидация, query/тела запросов), `httptest`
   ...
   Если атом по плану осознанно откладывает тесты (например, httptest обёрток —
   в следующий атом этапа), покрытие может временно оказаться ниже порога: это
   фиксируется отклонением в комментарии задачи, а порог подтверждается при
   закрытии этапа (пример: #23 → 69.1%, восстановление в #24).
